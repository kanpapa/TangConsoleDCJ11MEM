...
module top(
    ...
    output	  GPIO_TX,
    input	  GPIO_RX,
    output	  LOG_TX,    // (= pmod1[0]) for access log 
...
//---------------------------------------------------------------------------
  ...
  reg  RESET_n; // reset for UART and SD memory
...
//---------------------------------------------------------------------------
// Bus error
// read 760000-760077 or 777700 causes bus error
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
  ...
  assign PRS_ERROR = (sd2_error != 0);
  assign PPS_ERROR = (sd2_error != 0) | 1'b1; // disable punch
...
//     19456-25599: RK3
//     ...
//     (1024*n*6144)-(1024*n*6144+6143):RKn
//     44032-50175: RK7
//     50176-     : (reserved for RL11)
//
...
//---------------------------------------------------------------------------
  ...
  wire	 ack_IRQ_RFRK = (aio_code == AIO_INTACK && iack_level == LV_IRQ1);
...
//---------------------------------------------------------------------------
  ...
  wire [3:0]  sd2_error;
  wire [4:0]  sd2_state;
  wire [30:0] sd2_tape_address;
  wire [30:0] sd2_tape_address_write;
   
  assign sd2_dat1 = 1'bz;
  assign sd2_dat2 = 1'bz;
  ...
  (
   ...
   .i_tape_flush       (sw2),
   .o_sd_state         (sd2_state),
   .o_sd_error         (sd2_error),
   .o_sd_tape_address  (sd2_tape_address),
   .o_sd_tape_address_write (sd2_tape_address_write)
...
//  else if ( REG_RKCS[11] ) // Inhibit incrementing RKBA
  ...
  always @(posedge sys_clk) begin
     ...
     {led_array_r[38], led_array_g[38], led_array_b[38]}
       <= {(sd_error != 0), (sd2_error != 0), ALE_monitor};
     ...
     {led_array_r[36], led_array_g[36], led_array_b[36]}
       <= {diag_test_failed, HALT, ~INIT_n};
     {led_array_r[35], led_array_g[35], led_array_b[35]}
       <= {|memaccess[6:4], |memaccess[3:2], |memaccess[1:0]};
     {led_array_r[34], led_array_g[34], led_array_b[34]}
       <= {memaccess[7], 1'b0, 1'b0};
     ...
     case ( sw1_count )
       1: begin
	  led_array_r[31:24]  <= {sd_det_n, sd_state};
	  led_array_r[23:16]  <= sd_error;
	  led_array_r[15:8]   <= {sd2_det_n, sd2_state};
	  led_array_r[7:0]    <= sd2_error;
	  led_array_g[31:0]   <= 0;
	  led_array_b[31:0]   <= 0;
       end
       2: begin
	  led_array_r[31:0]   <= sd2_tape_address;
	  led_array_g[31:0]   <= 0;
	  led_array_b[31:0]   <= sd2_tape_address_write;
       end
       3: begin
	  led_array_r[31:16]  <= ADRS_DBG0 & 16'hffff;
	  led_array_r[15:0]   <= REG_DBG0;
	  {led_array_g[31:0], led_array_b[31:0]} <= 0;
       end
       4: begin
	  led_array_r[31:16]  <= ADRS_DBG1  & 16'hffff;
	  led_array_r[15:0]   <= REG_DBG1;
	  {led_array_g[31:0], led_array_b[31:0]} <= 0;
       end
       5: begin
	  led_array_r[31:16]  <= ADRS_DBG2  & 16'hffff;
...
//---------------------------------------------------------------------------
`define USE_ACCESSLOG    // enable hard disk access log module
