...
//  else if ( REG_RKCS[11] ) // Inhibit incrementing RKBA
  ...
  always @(posedge sys_clk) begin
     ...
     {led_array_r[33], led_array_g[33], led_array_b[33]} <= 0;
     
     {led_array_r[32], led_array_g[32], led_array_b[32]} <= sw2_count;
     case ( sw2_count )
       1: begin
	  ...
	  led_array_g[31:0]   <= sd2_tape_address;
	  led_array_b[31:0]   <= 0;
       end
       2: begin
	  led_array_r[31:16]  <= ADRS_DBG0 & 16'hffff;
	  led_array_r[15:0]   <= REG_DBG0;
	  {led_array_g[31:0], led_array_g[31:0]} <= 0;
       end
       3: begin
